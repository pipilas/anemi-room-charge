...
class UpdateAvailableDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        if self.mandatory:
            tk.Label(body,
                     text=f"You must update to continue using\n{app_name}.",
            ...
            tk.Label(body, text=f"{app_name} v{latest} is available",
        ...
        if notes:
            ...
            notes_frame = tk.Frame(body, bg="#F9FAFB",
        ...
        update_lbl = tk.Label(update_btn, text=btn_text, bg=ACCENT,
...
class DownloadAndInstallDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill="x")
        hdr.pack_propagate(False)
        self.hdr_label = tk.Label(hdr, text="Downloading Update...",
                                  bg=ACCENT, fg="#FFFFFF",
                                  font=(FONT, 13, "bold"))
    ...
    def _switch_to_install_phase(self):
        ...
        style = ttk.Style()
        style.configure("Update.Horizontal.TProgressbar", background=SUCCESS)
